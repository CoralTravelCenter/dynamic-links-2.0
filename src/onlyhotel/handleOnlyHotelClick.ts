...
export async function handleOnlyHotelClick(target: HTMLElement): Promise<any> {
	...
	const depthAttr = target.getAttribute("data-onlyhotel-lookup-depth-days");
	const nightsAttr = target.getAttribute("data-onlyhotel-lookup-nights");
	...
	const filters: string | null = target.getAttribute("data-onlyhotel-lookup-filter") ?? null;
