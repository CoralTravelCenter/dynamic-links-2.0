 ...
 */
export function extractOnlyHotelParams(target: HTMLElement) {
	...
	const hotelNames = raw.split(",").map(s => s.trim()).filter(Boolean);
	...
	const filters = target.getAttribute("data-onlyhotel-lookup-filter-2") ?? null;
